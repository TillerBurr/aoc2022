...
def parse_input(_input: str) -> tuple[list[Monkey], Counter]:
    ...
    for i, monkey in enumerate(monkey_list):
        ...
        monkeys.append(Monkey(d, func, test, if_true, if_false))
    return (monkeys, c)


def process_turn(
    monkey_num: int, monkeys: list[Monkey], counter: Counter
) -> tuple[list[Monkey], Counter]:
    monkey = monkeys[monkey_num]
    while monkey.items:
        item = monkey.items.popleft()
        item = monkey.func(item) // 3
        if monkey.test(item):
...
def process_round(
    monkeys: list[Monkey], counter: Counter
) -> tuple[list[Monkey], Counter]:
    for i in range(len(monkeys)):
        monkeys, counter = process_turn(i, monkeys, counter)
    return monkeys, counter


def process_mult_rounds(
    monkeys: list[Monkey], num_rounds: int, counter: Counter
) -> tuple[list[Monkey], Counter]:
    for i in range(num_rounds):
        monkeys, counter = process_round(monkeys, counter)
...
def calculate_monkey_inspections(
    monkeys: list[Monkey], counter: Counter, num_rounds: int
) -> int:
    process_mult_rounds(monkeys, num_rounds, counter)
...
def test(_input: str, expected: int) -> None:
    ...
    assert calculate_monkey_inspections(monkeys, counter, 20) == expected


if __name__ == "__main__":
    monkeys, counter = parse_input(data)

    print(f"Input:{calculate_monkey_inspections(monkeys,counter,20)}")
